...
import { CheckCircle, MessageCircle, ShoppingBag, Plus, Minus, X, Search, ShoppingCart } from "lucide-react";
...
function CatalogModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-[60] p-0 sm:p-4">
      <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-3xl shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
          <div>
            <h2 className="font-bold text-gray-900 text-lg">Catálogo de productos</h2>
            <p className="text-xs text-gray-400">{items.length} productos disponibles</p>
          </div>
        ...
        <div className="px-5 py-3 border-b border-gray-100 flex-shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-mk-pink/30"
            />
          </div>
        </div>
        ...
        <div className="overflow-y-auto flex-1 p-5">
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {filtered.map(({ product, price }) => {
                const inCart = cart.find((c) => c.productId === product.id);
                return (
                  <div key={product.id} className="bg-gray-50 rounded-2xl p-3 flex flex-col">
                    ...
                    <p className="text-sm font-semibold text-gray-900 leading-tight line-clamp-2 mt-0.5 flex-1">{product.name}</p>
                    {price && <p className="text-sm font-bold text-mk-pink mt-1">{fmt(price)}</p>}
                    <div className="mt-2">
                      {inCart ? (
                        <div className="flex items-center justify-between gap-1">
                          <button onClick={() => onUpdateQty(product.id, -1)} className="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center hover:bg-gray-100 transition-colors">
                            <Minus size={13} />
                          </button>
                          <span className="text-sm font-bold text-gray-900">{inCart.quantity}</span>
                          <button onClick={() => onUpdateQty(product.id, 1)} className="w-8 h-8 rounded-full mk-gradient flex items-center justify-center">
                        ...
                        <button
                          ...
                          className="w-full py-2 mk-gradient text-white text-xs font-semibold rounded-xl flex items-center justify-center gap-1 hover:opacity-90 transition-opacity"
                        >
        ...
        <div className="px-5 py-4 border-t border-gray-100 flex-shrink-0">
          <button onClick={onClose} className="w-full py-3 mk-gradient text-white font-bold rounded-2xl text-sm hover:opacity-90 transition-opacity">
            Listo · {cart.reduce((s, i) => s + i.quantity, 0)} producto(s) en pedido
          </button>
        </div>
      </div>
    </div>
...
export function PublicRequestForm({ consultant, mainProduct, mainProductPrice, catalogItems }: Props) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-rose-50 flex flex-col items-center justify-center p-6">
        <div className="bg-white rounded-3xl p-8 shadow-xl max-w-sm w-full text-center space-y-5">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">{consultant.name} te contactará pronto.</p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-rose-50">
      {/* Header */}
      <div className="mk-gradient px-5 py-3 flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center flex-shrink-0 p-1 shadow-sm">
      ...
      <div className="max-w-6xl mx-auto lg:grid lg:grid-cols-5 lg:gap-10 lg:px-10 lg:py-8">

        {/* ── Columna izquierda: producto + descripción ── */}
        <div className="lg:col-span-3">
          {/* Galería */}
          <div className="bg-white lg:rounded-2xl lg:overflow-hidden lg:shadow-sm">
            {mainProduct.images.length > 0 ? (
              <>
                <div className="aspect-square overflow-hidden bg-gray-50">
                  <img src={mainProduct.images[activeImg]} alt={mainProduct.name} className="w-full h-full object-contain" />
                </div>
                {mainProduct.images.length > 1 && (
                  <div className="flex gap-2 px-4 py-3 overflow-x-auto">
                    {mainProduct.images.slice(0, 8).map((img, i) => (
                      <button
                        key={i}
                        onClick={() => setActiveImg(i)}
                        className={`w-14 h-14 flex-shrink-0 rounded-xl overflow-hidden border-2 transition-all ${
                          i === activeImg ? "border-mk-pink scale-105" : "border-transparent opacity-50 hover:opacity-80"
                        }`}
                      >
          ...
          <div className="px-4 lg:px-0 pt-5 space-y-4">
            <div>
              ...
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 mt-1 leading-tight">{mainProduct.name}</h1>
              {mainProductPrice && (
            ...
            {mainProduct.generalInfo && (
              <div className="bg-gray-50 rounded-2xl p-4">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Información general</p>
                <p className="text-sm text-gray-700 leading-relaxed">{mainProduct.generalInfo}</p>
              </div>
            )}
            ...
            {catalogItems.length > 0 && (
              <button
                ...
                className="w-full flex items-center justify-between px-5 py-4 bg-white rounded-2xl border-2 border-mk-pink/20 hover:border-mk-pink/50 hover:bg-mk-pink-light transition-all group lg:mb-8"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="text-sm font-bold text-gray-900">Ver más productos</p>
                    <p className="text-xs text-gray-400">{catalogItems.length} productos disponibles</p>
                  </div>
                </div>
                <Plus size={18} className="text-mk-pink group-hover:scale-110 transition-transform" />
              </button>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 px-4 lg:px-0 pb-10 space-y-5 lg:pt-0 pt-5">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              {cart.map((item) => (
                <div key={item.productId} className="px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.price && <p className="text-xs text-mk-pink font-semibold">{fmt(item.price * item.quantity)}</p>}
                  </div>
            ...
            {catalogItems.length > 0 && (
              <div className="px-4 py-2 border-t border-gray-100">
                <button onClick={() => setShowCatalog(true)} className="w-full py-2 text-mk-pink text-xs font-semibold hover:bg-mk-pink-light rounded-xl transition-colors flex items-center justify-center gap-1">
